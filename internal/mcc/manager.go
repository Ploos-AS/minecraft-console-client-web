...
func (m *Manager) subscribe() (<-chan browserMessage, chan browserMessage, func()) {
	...
	return ch, ch, func() { m.mu.Lock(); delete(m.subscribers, ch); m.mu.Unlock() }
}
...
func (m *Manager) ServeBrowser(ctx context.Context, browser *websocket.Conn) error {
	events, reply, unsubscribe := m.subscribe()
	defer unsubscribe()
	...
	go func() {
		for {
			...
			if mt != websocket.TextMessage {
				nonblockingReply(reply, protocolErrorMessage("only text messages are supported"))
				continue
			}
			command, err := parseBrowserCommand(payload)
			if err != nil {
				nonblockingReply(reply, protocolErrorMessage(err.Error()))
				continue
			}
			if m.Status().State != StateConnected {
				nonblockingReply(reply, commandResponseMessage(command.ID, CommandResponse{Success: false, Message: "MCC is not connected"}))
				continue
			}
			if err := m.send(ctx, commandRequest{command: command, reply: reply}); err != nil {
				return
			}
		}
	}()
